...
//        port_=Settings.getString("Port","");
        ...
        double scannedCount = sales.getDoubleExtra("countScanned", 0);

        txtTotalScan.setText(scannedCount + " " + mUnitAssortment);
        ...
        if (Remain_in_program != 0.0){
            double remain_scan=  Remain_in_program - scannedCount;
            if (remain_scan<=0) {
            ...
            txtRemainScan.setText("0 " + mUnitAssortment);
            double surplus_scan = Math.abs(Remain_in_program - scannedCount);
            txtSurplusScan.setText(String.valueOf(surplus_scan) + " " + mUnitAssortment);
        }
        if(toInt == WeightPrefix){
            if(mBarcodeAssortment.length() == 13){
